...
define(['contrail-list-model'], function(ContrailListModel) {
    var ConfigNodeDetailsServiceMonitorChartListModel = function(config) {
        var hostname = config['node'];
        var postData = monitorInfraUtils.
                        getPostDataForCpuMemStatsQuery(
                                monitorInfraConstants.CONFIG_NODE,"configServiceMonitor");
        var listModelConfig = {
            remote : {
                ajaxConfig : {
    ...
    return ConfigNodeDetailsServiceMonitorChartListModel;
});
