...
define([
        ...
        return {
            ...
            viewConfig: {
                ...
                dataParser: function(result) {
                    ...
                    obj['analyticsMessages'] = getAnalyticsMessages(analyticsNodeData);
    ...
    function getTemplateGeneratorConfig() {
        ...
        templateGeneratorConfig = templateGeneratorConfig.concat(
            (monInfraConstants.IS_NODE_MANAGER_INSTALLED)? 
                    [
                         {
                             key: 'processes',
                             label: 'Processes',
                         ...
                         },
                         {
                             key: 'analyticsProcessStatusList.' + 
                                 monInfraConstants.UVEModuleIds['COLLECTOR'],
                             label: 'Collector',
                             templateGenerator: 'TextGenerator'
                         },
                         {
                             key: 'analyticsProcessStatusList.' + 
                                 monInfraConstants.UVEModuleIds['QUERYENGINE'],
                             label: 'Query Engine',
                             templateGenerator: 'TextGenerator'
                         },
                         {
                             key: 'analyticsProcessStatusList.' + 
                                 monInfraConstants.UVEModuleIds['APISERVER'],
                             label: 'API Server',
                             templateGenerator: 'TextGenerator'
                         }
                    ]
    ...
    function getAnalyticsMessages(aNodeData) {
        var msgs = monInfraUtils.getAnalyticsMessagesCountAndSize(aNodeData,['contrail-collector']);
        ...
        if(processStateList != null){
            for(var i=0; i < processStateList.length; i++){
                ...
                if (currProc.process_name == "contrail-query-engine"){
                    ret[monInfraConstants.UVEModuleIds['QUERYENGINE']] = getProcessUpTime(currProc);
                }  else if (currProc.process_name == "contrail-analytics-nodemgr"){
                    ret[monInfraConstants.UVEModuleIds['ANALYTICS_NODEMGR']] = getProcessUpTime(currProc);
                }  else if (currProc.process_name == "contrail-analytics-api"){
                    ret[monInfraConstants.UVEModuleIds['APISERVER']] = getProcessUpTime(currProc);
                } else if (currProc.process_name == "contrail-collector"){
                    ret[monInfraConstants.UVEModuleIds['COLLECTOR']] = getProcessUpTime(currProc);
                } 
            }
        }
        return ret;
    ...
    function getAnalyticsNodeDetails(ctrlNodeData) {
        ...
        try{
           anlNode = jsonPath(
                   ctrlNodeData,"$..ModuleClientState..primary")[0].split(':')[0];
