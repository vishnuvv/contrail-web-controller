...
define(['contrail-graph-model'],function(ContrailGraphModel) {
   UnderlayGraphModel = ContrailGraphModel.extend({
       ...
       getElementsForUnderlayGraph : function (response) {
           ...
           //TODO need to handle the error nodes
           this.nodes = nodes;
       ...
       createElementsFromAdjacencyList: function () {
           ...
           _.each(adjacencyList, function(edges, parentElementLabel) {
               if(null !== elMap["node"][parentElementLabel] &&
                   ...
                   if(null !== el && typeof el !== "undefined") {
                       ...
                       el = jsonPath(conElements, '$[?(@.id=="' + elMap["node"][parentElementLabel] + '")]');
               ...
               if(false !== parentNode && parentNode.length === 1) {
                   ...
                   _.each(edges, function(childElementLabel) {
                       if(null !== elMap["link"][parentElementLabel + "<->" + childElementLabel] &&
                           ...
                           var linkEl = self.getCell(elMap["link"][parentElementLabel + "<->" + childElementLabel]);
                           if(null !== linkEl && typeof linkEl !== "undefined") {
                               linkElements.push(linkEl);
                               return;
                           } else {
                               linkEl = jsonPath(conElements, '$[?(@.id=="' + elMap["link"][parentElementLabel + '<->' + childElementLabel] + '")]');
                           ...
                           for(var i=0; i<links.length; i++) {
                               ...
                               if((link.endpoints[0] === childElementLabel && link.endpoints[1] === parentElementLabel) ||
                                   ...
                                   if((null == elMap["link"][linkName] && typeof elMap["link"][linkName] == "undefined") &&
                                       ...
                                       linkElements.push(self.createLink(link, link_type, parentId, childId));
           ...
           for(var i=0; i<links.length; i++) {
               ...
               if(null !== elMap["node"] && typeof elMap["node"] !== "undefined") {
                   ...
                   if(null == elMap["node"][endpoint1] && typeof elMap["node"][endpoint1] == "undefined") {
                   ...
                   if((null == elMap["link"][linkName] && typeof elMap["link"][linkName] == "undefined") ||
                       ...
                       if(false !== parentNode && parentNode.length === 1) {
                           ...
                           if(false !== childNode && childNode.length === 1) {
                               ...
                               var link_type = parentNodeType.split("-")[0][0] + parentNodeType.split("-")[1][0] + '-' + 
                                   childNodeType.split("-")[0][0] + childNodeType.split("-")[1][0];
                               var linkEl = this.createLink(link, link_type, parentId, childId);
                       ...
                       linkEl = jsonPath(linkElements, '$[?(@.id=="' + elMap["link"][linkName] + '")]');
                       if(typeof linkEl === "object" && linkEl.length === 1) {
                           ...
                           if(null !== linkEl && typeof linkEl !== "undefined") {
                               ...
                               // exists in adjList
                               linkEl = jsonPath(conElements, '$[?(@.id=="' + elMap["link"][linkName] + '")]');
                               if(typeof linkEl === "object" && linkEl.length === 1) {
                                   if(adjacencyList.hasOwnProperty(endpoint0) &&
                                       adjacencyList.hasOwnProperty(endpoint1)) {
                                       linkElements.push(linkEl[0]);
       ...
       addElementsToGraph: function (els, clickedElement) {
           ...
           paper.setOrigin(0,0);
           $('#' + ctwl.UNDERLAY_GRAPH_ID).prop('style').removeProperty('transform');
           $('#' + ctwl.UNDERLAY_GRAPH_ID).offset({
               "top" : $('#' + ctwl.UNDERLAY_GRAPH_ID).parent().offset().top,
               "left": $('#' + ctwl.UNDERLAY_GRAPH_ID).parent().offset().left
           });
           this.addCells(els);
           var newGraphSize = joint.layout.DirectedGraph.layout(this, {"rankDir" : "TB", "nodeSep" : 50, "rankSep" : 50});
           ...
           if(svgWidth > viewAreaWidth) {
               if(typeof clickedElement !== "undefined" && null !== clickedElement) {
                   var fixedDivPosition = $('#' + ctwl.UNDERLAY_GRAPH_ID).parent().offset();
                   var fixedDivWidth = $('#' + ctwl.UNDERLAY_GRAPH_ID).parent().width();
                   var fixedDivHeight = $('#' + ctwl.UNDERLAY_GRAPH_ID).parent().height();
           ...
           //TODO Need to handle error nodes functionality
