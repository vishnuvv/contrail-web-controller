...
define([
    ...
    var UnderlayTabView = ContrailView.extend({
        ...
        render: function () {
            ...
            //$(self.$el).parent().append(contrail.getTemplate4Id(cowc.TMPL_LOADING_TEXT)());
            $(self.$el).hide();
            this.renderView4Config(self.$el, self.model, getUnderlayTabConfig(viewConfig), null, null, null, function () {
                //$(self.$el).parent().find('.graph-loading').hide();
                $(self.$el).show();
                var contrailTabObj = $(self.$el).find('#contrail-tabs').data('contrailTabs');
                contrailTabObj.disableTab(
                    ctwc.UNDERLAY_PROUTER_TAB_INDEXES.concat(
                        ctwc.UNDERLAY_LINK_TAB_INDEX,
                        ctwc.UNDERLAY_VM_TAB_INDEXES,
    ...
    var getUnderlayTabConfig = function (viewConfig) {
        ...
        // Instance tab config (Details, Interfaces, Traffic Statistics, Port Distribution, Port Map, CPU/Memory)
        var instanceTabConfig = ctwvc.getInstanceDetailPageTabConfig(viewConfig);
        
        // Underlay default tab config (Search flows & Trace flow)
        var underlayDefaultTabConfig = ctwvc.getUnderlayDefaultTabConfig(viewConfig);
        
        // pRouter tab config (Interface tab, Traffic statistics tab, Details tab)
        var underlayPRouterTabConfig = ctwvc.getUnderlayPRouterTabConfig(viewConfig);
        
        // TrafficStatistics tab for prouter  link
        var underlayPRouterLinkTabConfig = ctwvc.getUnderlayPRouterLinkTabConfig(viewConfig);
        ...
        var underlayVRouterTabConfig = ctwvc.getVRouterDetailsPageTabs(vRouterViewConfig);
        
        var underlayTabConfig = underlayDefaultTabConfig.concat(underlayPRouterTabConfig,
                                underlayPRouterLinkTabConfig, instanceTabConfig,
                                underlayVRouterTabConfig);
        return {
                                ...
                                viewConfig: {
                                    ...
                                    activate: function (e, ui) {
                                        ...
                                        if (selTab == ctwl.UNDERLAY_TRACEFLOW_TITLE) {
                                            ...
                                            $('#' + ctwl.INSTANCE_PORT_DIST_CHART_ID).
                                            trigger('refresh');
